...
async def handler(message: types.Message) -> None:
    if message.text.startswith("."):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(f"https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=AIzaSyCxscNfFRCcbIMV078K5QdUk09OoujG8tY", json=data) as response:
                try:
